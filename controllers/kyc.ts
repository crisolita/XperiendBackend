...
try {
   ...
   const {name,lastname,country_born,birth,estado_civil,
    regimen_matrimonial,telf,address,document,nivel_inversion,document_number,city,postalCode,state,country,foto_dni_frontal,foto_dni_trasera,wallet, cuenta_bancaria ,residencia_fiscal}= req.body;
   const user= await getUserById(USER.id,prisma)
   const kycAlready= await getKycInfoByUser(USER.id,prisma)
   const alreadyWallet= await prisma.kycInfo.findUnique({where:{wallet}})
   let info,dataImages=[];
   if(!user) return res.status(404).json({error: "User no encontrado"})
   if(alreadyWallet) return res.status(400).json({error: "Wallet ya ha sido registrada"})
   ...
   if(kycAlready?.status=="APROBADO" || kycAlready?.status=="PENDIENTE") { 
    ...
    return res.status(400).json({error:"Kyc Rechazado, actualizar datos"})
     ...
     let base64Frontal,pathFrontal,pathTrasera,base64Trasera,base64Fiscal,pathFiscal
   ...
   dataImages.push(img)
    ...
    dataImages.push(img)
     ...
     if(residencia_fiscal ) {
      ...
      base64Fiscal= residencia_fiscal.replace(/^data:image\/(png|jpg|jpeg);base64,/, '')
...
export const updateKYC= async (req:Request,res: Response) => {
  try {
     ...
     const {name,lastname,estado_civil,
      regimen_matrimonial,residencia_fiscal,country_born,birth,telf,address,document_number,document,city,postalCode,state,country,foto_dni_frontal,foto_dni_trasera,wallet,prueba_titularidad }= req.body;
     const user= await getUserById(USER.id,prisma)
     const kycAlready= await getKycInfoByUser(USER.id,prisma)
     let info,img;
     if(!user) return res.status(404).json({error: "User no encontrado"})
    ...
    if(foto_dni_frontal) {
          ...
          const image= await prisma.kycImages.findFirst({where:{rol:"RESIDENCIA_FISCAL"}})
          if(residencia_fiscal && image ) {
            const pathFiscal=`kyc_image_${user.id}_${info.id}_${"RESIDENCIA_FISCAL"}`
      
            const base64Fiscal= residencia_fiscal.replace(/^data:image\/(png|jpg|jpeg);base64,/, '')
            let data= Buffer.from(base64Fiscal,'base64')
            await uploadImage(data,pathFiscal)
            ...
            await uploadImage(data,pathFiscal)
             ...
             if(prueba_titularidad && image ) {
              const pathTitularidad=`kyc_image_${user.id}_${info.id}_${"TITULARIDAD"}`
        
              const base64titular= prueba_titularidad.replace(/^data:image\/(png|jpg|jpeg);base64,/, '')
