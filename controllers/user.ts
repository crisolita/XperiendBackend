...
import {
  sendAuthEmail,
  sendRecoverCode,
  sendWelcomeEmailConSubs,
  sendWelcomeEmailSinSubs,
} from "../service/mail";
...
export const userRegisterController = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ...
      res
        .status(200)
        .json({ email: email, referallCode: userName, userName: userName });
    } else {
...
export const userGoogleController = async (req: Request, res: Response) => {
  try {
    ...
    if (!response.data || !response.data.verified_email || !userName)
    ...
    if (!exist) {
      ...
      res.status(200).json({
        ...
        referallFriend: exist.referallFriend,
        kycStatus: exist.kycStatus,
...
export const changeNewsletter = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ email: user?.email, newsletter: userUpdate?.newsletter });
  } catch (error) {
    console.log(error);
    return res.status(500).json(error);
  }
...
export const setFavorite = async (req: Request, res: Response) => {
  try {
    ...
    user = await prisma.user.update({
      where: { id: USER.id },
      data: { favoritos: [...user.favoritos, project_id] },
    });
...
export const userCanBuy = async (req: Request, res: Response) => {
  try {
    ...
    const fecha_inicial = await getFechaDeVentaInicial(
      kyc.wallet,
      Number(project_id),
      prisma
    );
