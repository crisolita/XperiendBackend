...
export async function sendWelcomeEmailConSubs(email: string,username:string) {
  const plantilla=fs.readFileSync("emails/emails-user/Gestion User - Registro bienvenida con suscripcion.html", 'utf-8')
  const data={
    username
  }
  const mailData = {
...
export async function sendKycRellenado(email: string,username:string) {
  const plantilla=fs.readFileSync("emails/emails - Gestion KYC/Gestion KYC - Rellenado.html", 'utf-8')
  const data={
    username
  }
  const mailData = {
...
export async function sendTransferenciaRecibidaParticipaciones(email: string,username:string) {
  const plantilla=fs.readFileSync("emails/Gestion Investhome Venta/Venta INVESTHOME - Transferencia recibida.html", 'utf-8')
  const data={
    username
  }
  const mailData = {
...
export async function intercambioTransferenciaPendiente(email: string,username:string, banco:string,
